...
2. **Triages and classifies** — keeps what's worth remembering (decisions, facts, people, research, things you made), ignores the chatter, and for each keeper works out (by its *content*, not by which folder was open) which context-vault it belongs to. It saves the full source into that vault's `raw/`. If it can't confidently tell which vault an item is about, it parks a small stub in `_inbox/` for you to confirm with one tap — it never guesses a vault and never drops an item.
3. Compiles the keepers into clean memory pages, first filed into the right vault (context), then under the right topic within that vault, following your memory rulebook.
4. Writes a short line in that vault's history log so you can see what it added.
...
2. The first time it runs, it may ask permission to read your sessions and write to your folder — approve once, and it's remembered. (On that first run it may also ask which contexts, if any, you want kept private, so it knows which vaults to keep out of cross-vault surfacing.)
...
You can also glance at `_inbox/` for anything the sweep left awaiting a routing decision (a one-tap confirm of which vault it belongs to).
