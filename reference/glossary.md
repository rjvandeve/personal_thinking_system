...
**Memory / knowledge base** — the organized store of what the assistant knows about your world. It grows and improves as you feed it. It's a **multi-vault brain**: many separate vaults, one shared mind on top.

**Brain (brain root)** — the single folder you connect to. It doesn't hold your notes directly; it holds one vault per context (under `vaults/`), the shared cross-vault map (`_ontology/`), the inbox (`_inbox/`), and the root rulebook.

**Context-vault (a vault)** — one physically separate store for one world in your life (a company, a client, a board seat, personal life). Its own files, its own map, openable on its own in Obsidian. Keeping them physically separate *is* how privacy works here.

**Inbox (`_inbox/`)** — where the nightly sweep parks anything it can't confidently assign to a vault, so you can confirm with one tap. It never guesses a vault or throws an item away.

**Private vault** — a context-vault marked `private: true`. It's kept up to date like any other, but sealed off from the shared map: never mixed into cross-context answers. The escape hatch for things that must never co-mingle.

**Cross-vault synthesis / bridge** — the payoff of one brain over many silos: the assistant reads across all your vaults to answer, and a *bridge* is an explicit link it records when the same idea, dependency, or contradiction shows up in two different worlds.

**Raw** — the original source material you feed in (a PDF, an email, notes). Never changed; kept as the record of truth. Each vault has its own `raw/`.
...
**Rulebook** — the `CLAUDE.md` files that tell the assistant how to keep your memory organized. There's a *root* rulebook at the brain root (routing between vaults, synthesis across them, and the capability ledger) and one *per-vault* rulebook in each context-vault (its own rules and its private flag). They apply automatically; you don't read them for fun.
...
**Nightly memory sweep** — a scheduled job that runs each night, reviews your recent conversations and new documents, decides *which context-vault* each item belongs to (by its content), and files what's worth remembering into that vault. Anything it can't place confidently goes to the inbox for a one-tap confirm. It means you don't have to remember to feed everything in by hand.
...
**Ontology** — the *one shared* vocabulary your whole brain is organized around, living in `_ontology/`: the agreed list of the kinds of things in your world and how they relate. Because it spans every vault, it's what lets physically separate vaults behave as a single mind, and it keeps every page filed consistently instead of each note inventing its own categories. Private vaults are left out of it.
