...
4. **Register every capability (mandatory).** At the END of each phase, update the user's **root memory file** (`CLAUDE.md` in their vault) — its "Active Capabilities" ledger — with the new capability and an imperative USE directive. A capability that exists but isn't registered will be ignored by future sessions. This is how each layer actually gets used. See `COACH.md` → "Capability Registration."
5. **Be resumable.** Maintain `state/PROGRESS.md` in the user's vault (copy from `state/PROGRESS.template.md`). On every session, read it first and resume where they left off.
...
- `templates/` — files you instantiate (incl. `USER-CLAUDE.md.template`, the user's root memory file with the Capability Ledger).
